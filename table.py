...
def Database():
    ...
    cursor = con.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS admin(id Integer Primary Key, name text, phone_number interger, classs integer, section text, fees_details text)")
    con.commit()
    con.close()
...
lblphone.grid(row=2, column=0, padx=10, pady=20,sticky="w")
txtphone = Entry(entry_frame, font=("times new roman","15","bold"), textvariable=phone_number, width=30)
txtphone.grid(row=2, column=1, padx=10, pady=20,sticky="w")

lblclass=Label(entry_frame, text="CLASS",font=("times new roman","15","bold"), width=10)
lblclass.grid(row=2, column=2,padx=10,pady=20,sticky="w")
txtclass = Entry(entry_frame, font=("times new roman","15","bold"), textvariable=classs, width=30)
txtclass.grid(row=2, column=3,padx=10,pady=20,sticky="w")

lblsection=Label(entry_frame, text="SECTION",font=("times new roman","15","bold"), width=10)
lblsection.grid(row=3, column=0,padx=10,pady=20,sticky="w")
txtsection = Entry(entry_frame, font=("times new roman","15","bold"), textvariable=section, width=30)
txtsection.grid(row=3, column=1,padx=10,pady=20,sticky="w")

lblfees_details =Label(entry_frame, text="FEES",font=("times new roman","15","bold"), width=10)
lblfees_details.grid(row=3, column=2,padx=10,pady=20,sticky="w")
txtfees_details = Entry(entry_frame, font=("times new roman","15","bold"), textvariable=fees_details, width=30)
txtfees_details.grid(row=3, column=3,padx=10,pady=20,sticky="w")
...
def add():
    con = sqlite3.connect("StudentManagement.db")
    cursor = con.cursor()
    cursor.execute("INSERT INTO admin VALUES ('%s','%s','%s','%s','%s','%s')"% (id.get(),name.get(), phone_number.get(), classs.get(), section.get(), fees_details.get()))
    con.commit()
...
def update():
    con = sqlite3.connect("StudentManagement.db")
    cursor = con.cursor()
    cursor.execute("UPDATE admin SET fees_details='%s' WHERE id='%s';" % (fees_details.get(), id.get()))
    con.commit()
    messagebox.showinfo("Notification", "Data Updated Successfully")
def delete():
    con = sqlite3.connect("StudentManagement.db")
    cursor = con.cursor()
    cursor.execute("DELETE FROM admin WHERE id='%s';" %  (id.get()))
    con.commit()
...
tv = Treeview(tree_frame, columns=(1,2,3,4,5,6))
